...
public class ProductoService {
    ...
    // En el método insertarProducto, agregar validación de código único
    public boolean insertarProducto(Producto producto) {
        // Verificar que el código de barras no exista (si se proporciona)
        if (producto.getCodebar() != null && !producto.getCodebar().trim().isEmpty()) {
            Producto existente = obtenerPorCodigoBarras(producto.getCodebar());
            if (existente != null && !existente.getIdproducto().equals(producto.getIdproducto())) {
                System.err.println("❌ Ya existe un producto con ese código de barras");
                return false;
            }
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Si es duplicado, no contar como error
            if (!e.getMessage().contains("UNIQUE constraint failed")) {
                System.err.println("Error insertando producto " + producto.getIdproducto() + ": " + e.getMessage());
            }
            return false;
        }
    }
